    ...
    def _get_sample_model(self):
        ...
        for t in range(max_blocks):
            ...
            if t == early_stop_at:
                ...
                action_probs = logits.softmax(0)
                if t >= self.min_blocks:
                    action_probs = action_probs * (1 - self.random_action_prob) + self.random_action_prob / logits.shape[0]
                else:
                    action_probs[0:2] = action_probs[0:2].sum()
                    action_probs = action_probs * (1 - self.random_action_prob) + self.random_action_prob / (logits.shape[0] - 1)
                action_logit = action_probs.log()[action]
            ...
            if t >= self.min_blocks and action == 0:
                ...
                samples.append(((m,), ((-1,0),), r, None, 1, action_logit))
                break
            else:
                ...
                if len(m.blocks) and not len(m.stems) or t == max_blocks - 1:
                    ...
                    if self.ignore_parents:
                        samples.append(((m_old,), (action,), r, m, 1, action_logit))
                    else:
                        samples.append((*zip(*self.mdp.parents(m)), r, m, 1, action_logit))
                    break
                else:
                    if self.ignore_parents:
                        samples.append(((m_old,), (action,), 0, m, 0, action_logit))
                    else:
                        samples.append((*zip(*self.mdp.parents(m)), 0, m, 0, action_logit))
...
class DatasetDirect(Dataset):
    ...
    def sample2batch(self, mb):
        ...
        return (s, a, r, d, n, mols, sl, idc, lens)
...
def train_model_with_proxy(args, model, proxy, dataset, num_steps=None, do_save=True):
    ...
    for i in range(num_steps):
        ...
        if args.objective == 'fm':
            ...
            s, a, r, d, n, mols, sl, idc, lens, *o = minibatch
            stem_out_s, mol_out_s = model(s, None)
            # index parents by their corresponding actions
            ...
            if args.objective == 'tb':
                ...
                loss = losses.mean()
            if args.objective == 'hvi': # reverse KL
                uniform_log_PB = tzeros.index_add(0, idc, torch.log(1/n))
                traj_logits = tzeros.index_add(0, idc, logits)
                #print(sl)
                traj_sample_logits = tzeros.index_add(0, idc, torch.Tensor([ x for x in sl]).to(dtype=args.floatX,device=logits.device))
                weights = (traj_logits-traj_sample_logits).softmax(0).detach()
                rewards = (traj_logits - (torch.log(traj_r) + uniform_log_PB)).detach()
                losses = ( rewards - rewards.mean() ) * traj_logits
                loss = (losses*weights).sum() / weights.sum()
            if args.objective == 'fkl': # forward KL
                uniform_log_PB = tzeros.index_add(0, idc, torch.log(1/n))
                traj_logits = tzeros.index_add(0, idc, logits)
                traj_sample_logits = tzeros.index_add(0, idc, torch.Tensor([ x for x in sl]).to(dtype=args.floatX,device=logits.device))
                weights = (torch.log(traj_r) + uniform_log_PB - traj_sample_logits).softmax(0).detach()
                losses = (-traj_logits * weights).sum()
                print(weights)
                loss = losses.sum()
...
if __name__ == '__main__':
  ...
  if 0:
    ...
    all_hps = [
      ...
      for rexp in [4, 8, 10, 16]
      ]
    if args.print_array_length:
      print(len(all_hps))
    else:
      args.run = args.run+args.shift  
